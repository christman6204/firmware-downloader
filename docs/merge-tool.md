...
- 依赖 `hex_utils.py`（Intel HEX 解析库，本仓库自带）
...
```
tool/merge_tool.py       GUI 工具（单文件）
tool/hex_utils.py        依赖的 HEX 解析库（本仓库自带）
```
...
配置文件：`tool/.merge_tool_config.json`
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.1 | 2026-08-09 | 路径更正 `tools/`→`tool/`；补充 hex_utils.py 依赖说明；澄清验证方法假设 |
| v1.0 | 2026-08-02 | 初版
